...
class GUI(Tk):
    ...
    def __init__(self):
        ...
        self.canvas = Canvas(self, width=600, height=550)
        ...
        Label(self, text="eps_h1").pack(side=LEFT)
        self.eps_h1 = DoubleVar()
        self.eps_h1.set(.1)
        Entry(self, textvariable=self.eps_h1, width=5).pack(side=LEFT)

        Label(self, text="eps_h2").pack(side=LEFT)
        self.eps_h2 = DoubleVar()
        self.eps_h2.set(.1)
        Entry(self, textvariable=self.eps_h2, width=5).pack(side=LEFT)
        ...
        [self.energy, self.norm_grad, self.prediction, _, self.mse, _, _, _, _, _, _] = self.net.iterate(lambda_x=0., lambda_y=0., epsilon_x=0., epsilon_h1=0., epsilon_h2=0., epsilon_y=0., alpha_W1=0., alpha_W2=0., alpha_W3=0.)
    ...
    def update_canvas(self, first_time = False):
        ...
        h1_mat = 256*self.net.h1.get_value().reshape((10,self.net.n_hidden[0]/10))
        h1_img=Image.fromarray(h1_mat).resize((self.net.n_hidden[0]/2,50))
        self.h1_imgTk=ImageTk.PhotoImage(h1_img)

        h2_mat = 256*self.net.h2.get_value().reshape((10,self.net.n_hidden[1]/10))
        h2_img=Image.fromarray(h2_mat).resize((self.net.n_hidden[1]/2,50))
        self.h2_imgTk=ImageTk.PhotoImage(h2_img)
        ...
        if first_time:
            ...
            self.h1_img_canvas             = self.canvas.create_image(400, 150, image = self.h1_imgTk)
            self.h2_img_canvas             = self.canvas.create_image(400, 220, image = self.h2_imgTk)
            self.x_img_canvas              = self.canvas.create_image(400, 320, image = self.x_imgTk)
            self.x_data_img_canvas         = self.canvas.create_image(400, 470, image = self.x_data_imgTk)
            ...
            self.canvas.itemconfig(self.h1_img_canvas,             image = self.h1_imgTk)
            self.canvas.itemconfig(self.h2_img_canvas,             image = self.h2_imgTk)
    ...
    def run(self):
        ...
        while True:
            ...
            eps_h1 = np.float32(self.eps_h1.get())
            eps_h2 = np.float32(self.eps_h2.get())
            eps_y  = np.float32(self.eps_y.get())

            [self.energy, self.norm_grad, self.prediction, _, self.mse, _, _, _, _, _, _] = self.net.iterate(lambda_x=lambda_x, lambda_y=lambda_y, epsilon_x=eps_x, epsilon_h1=eps_h1, epsilon_h2=eps_h2, epsilon_y=eps_y, alpha_W1=0., alpha_W2=0., alpha_W3=0.)
